...
from trl.trainer import DPOTrainer, VDPOTrainer, SVCOTrainer, TieDPOTrainer
...
def get_variable_length_grouped_indices(lengths, batch_size, world_size, megabatch_mult=8, generator=None):
    ...
    batches = [shuffled_indices[i : i + world_batch_size] for i in range(0, len(shuffled_indices), world_batch_size)]
        ...
        if self.is_fsdp_enabled:
            ...
            if is_accelerate_available("0.23.0"):
                ...
                if fsdp_plugin.activation_checkpointing and self.args.gradient_checkpointing:
                    raise ValueError("The activation_checkpointing in FSDP config and the gradient_checkpointing in training arg can't be set to True simultaneously.")
...
class LLaVATieDPOTrainer(TieDPOTrainer):
    """LLaVA wrapper for TieDPO with Tie Regularization."""
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            super(LLaVATieDPOTrainer, self)._save(output_dir, state_dict)
