...
class VDPOTrainer(Trainer):
    ...
    def get_seg_weight(self, win_labels, rej_labels,win_input_ids, rej_input_ids, diffusion_idx):
        #import pdb; pdb.set_trace()
        win_token_weight = torch.ones_like(win_labels[:, 1:], dtype=torch.bfloat16)
        rej_token_weight = torch.ones_like(win_labels[:, 1:], dtype=torch.bfloat16)
        #win_token_weight = torch.zeros_like(win_labels[:, 1:], dtype=torch.bfloat16)
        #rej_token_weight = torch.zeros_like(win_labels[:, 1:], dtype=torch.bfloat16)
        for idx, (w, r) in enumerate(zip(win_input_ids, rej_input_ids)):
            valid_w = w[1:]
            valid_r = r[1:]
            #w_mod = self.get_ids(valid_w.tolist(), target_sequence=[2461, 2168, 220])
            # import pdb; pdb.set_trace()
            w_mod = self.get_ids(valid_w.tolist(), target_sequence=[2461, 279, 12864])
            r_mod = self.get_ids(valid_r.tolist(), target_sequence=[2461, 279, 12864])
            #import pdb; pdb.set_trace()
            #w_mod= [item for i in diffusion_idx for item in w_mod[i]]
            # win_token_weight[idx][w_mod] = self.args.dpo_token_weight
            # rej_token_weight[idx][r_mod] = self.args.dpo_token_weight

            # Apply weight decay to win_token_weight and rej_token_weight
            decay_factor = 0.9  # Decay factor for weight adjustment
            win_token_weight, rej_token_weight = self.apply_weight_decay(w_mod, r_mod, win_token_weight, rej_token_weight, decay_factor, idx)
        return win_token_weight, rej_token_weight
    ...
    def compute_loss(self, model: Module, inputs: dict, return_outputs=False):
        ...
        output_logits, new_labels = model(
            ...
            images=images+images+diffusion_image,
            dpo_forward=True,
            image_sizes=image_sizes+image_sizes+diffusion_image_sizes,
            modalities=modalities+modalities+diffusion_modalities,
            **data_dict
        )
        ...
        if ref_logps is None:
            ...
            per_uncond_ref_token_logps = chip_get_batch_logps(
                output_logits, ref_output.logits, unconditional_ref_output.logits,
        ...
        policy_win_per_token_logps, policy_rej_per_token_logps,policy_win_diffusionImage_per_token_logps = per_policy_token_logps.split([win_size, rej_size, win_size])
        ...
        if self.args.dpo_token_weighted:
            win_token_weight, rej_token_weight = self.get_seg_weight(win_labels, rej_labels, win_input_ids, rej_input_ids, diffusion_idx)
            ...
            policy_chosen_logp_v = policy_chosen_logp
        ########################################
        policy_win_ps, policy_rej_ps, policy_win_diffusionImage_ps = policy_vocab_ps.split([win_size, rej_size, win_size])

        dpo_losses, chosen_rewards, rejected_rewards, v_dpo_losses, q_abs_chosen, q_abs_chosen_v, q_abs_rejected, q_abs_rejected_v, v_chosen_rewards, v_rejected_rewards= self.chip_loss(
